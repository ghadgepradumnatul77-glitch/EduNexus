                        ...
                        <form onSubmit={createUser} className="grid grid-cols-1 sm:grid-cols-2 gap-8 relative z-10">
                            <div>
                                ...
                                <input required value={form.first_name} onChange={e => setForm(f => ({ ...f, first_name: e.target.value }))}
                                    className="w-full bg-surface-main border border-edu-border rounded-xl px-4 py-3 text-sm text-text-primary focus:ring-4 focus:ring-primary-500/10 outline-none transition-all" />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}
                                    className="w-full bg-surface-main border border-edu-border rounded-xl px-4 py-3 text-sm text-text-primary focus:ring-4 focus:ring-primary-500/10 outline-none transition-all cursor-pointer">
                ...
                <div className="flex flex-wrap items-center gap-6">
                    <div className="relative w-full max-w-sm group">
                        <input
                            ...
                            className="w-full bg-surface-card border border-edu-border rounded-xl px-10 py-3 text-sm text-text-primary focus:ring-4 focus:ring-primary-500/10 outline-none transition-all placeholder:text-text-muted/50"
                        />
                    ...
                    <select
                        ...
                        className="bg-surface-card border border-edu-border rounded-xl px-6 py-3 text-sm text-text-primary focus:ring-4 focus:ring-primary-500/10 outline-none transition-all cursor-pointer font-bold"
                    >
                ...
                <div className="bg-surface-card rounded-[2rem] shadow-premium overflow-hidden border border-edu-border">
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="text-[10px] font-black text-text-muted uppercase tracking-widest border-b border-edu-border bg-surface-main/30">
                    ...
                    {/* Pagination */}
                    {totalPages > 1 && (
                        <div className="px-6 py-4 bg-surface-main/30 border-t border-edu-border flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                                className="px-4 py-2 rounded-xl border border-edu-border disabled:opacity-30 hover:bg-surface-main transition-all">← Prev</button>
                            <span className="text-text-muted">Page {page} of {totalPages}</span>
                            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                                className="px-4 py-2 rounded-xl border border-edu-border disabled:opacity-30 hover:bg-surface-main transition-all">Next →</button>
                        </div>
                    )}
                </div>
            </div>
